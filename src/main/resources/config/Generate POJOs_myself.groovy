...
def generate(table, dir) {
    ...
    def entityPath = "${dir.toString()}\\entity",
        ...
        daoPath = "${dir.toString()}\\dao"

    mkdirs([entityPath, servicePath, daoPath])
    ...
    new File("${daoPath}\\${entityName}DAO.java").withPrintWriter { out -> genRepository(out, table, entityName, fields, basePackage) }
...
def genService(out, table, entityName, fields, basePackage) {
    ...
    out.println ""
    out.println "import ${basePackage}.dao.${entityName}DAO;"
    ...
    out.println "\tprivate ${entityName}DAO rep;"
    out.println "}"
}
...
def genRepository(out, table, entityName, fields, basePackage) {
    ...
    out.println "/**\n" +
            " * @author Source\n" +
            " */"
    out.println "public interface ${entityName}DAO extends JpaRepository<$entityName, ${fields[0].type}>{"
    out.println "}"
}
