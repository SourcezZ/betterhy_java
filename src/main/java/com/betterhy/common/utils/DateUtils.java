...
public class DateUtils {
    ...
    /**
     * 得到当前日期和时间字符串.
     * @return String 日期和时间字符串，例如 20150811095153
     * @since 1.0
     */
    public static String getDateTimeNoSplit() {
        return formatDate(new Date(), DateUtils.DATETIME_FORMAT_NO_SPLIT);
    }
    ...
    public static Date parseDate(Object str, String parsePattern) {
        ...
        try {
            return org.apache.commons.lang.time.DateUtils.parseDate(str.toString(), new String[] {parsePattern});
        } catch (ParseException e) {
            return null;
        }
    }
    ...
    /**
     *  java 获取某年某月 所有日期（yyyy-mm-dd格式字符串）
     * @param year 年
     * @param month 月
     * @return List<String>
     */
    public static List<String> getMonthFullDay(int year, int month, Date endDate){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        ...
        for (int j = 1; j <= count; j++) {
            ...
            Date currentDate = DateUtils.getDate(date, "yyyy-MM-dd");
            if (currentDate != null && endDate != null && currentDate.after(endDate)){
                break;
            }
            fullDayList.add(date);
    ...
    public String getLastDay(int year,int month){
        ...
        return DateUtils.getDate(cal.getTime(), "yyyy-MM-dd");
    }
